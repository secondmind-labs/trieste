...
class EnsembleDataTransformer:
    ...
    This can be done by initializing the `EnsembleDataTransformer` with the `bootstrap_data` set to
    `True`.
    """
    ...
    def __init__(self, networks: List[KerasNetwork], bootstrap_data: bool = False):
        ...
        self.input_output_names: Optional[Dict[str, List[str]]] = None
    ...
    def __call__(self, data: Dataset) -> TrainingData:
        ...
        the Keras ensemble model. Note that it is crucial that the ensemble model populates
        `input_output_names` attribute with values after it has been compiled.
        ...
        assert (
            len(self.input_output_names["inputs"]) > 0
            and len(self.input_output_names["outputs"]) > 0
        )
    ...
    def ensemblise_inputs(
        self, query_points: TensorType, transform_data: bool = False
