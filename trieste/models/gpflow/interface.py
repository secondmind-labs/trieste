...
class FantasizedGPflowPredictor(ProbabilisticModel, tf.Module, ABC):
    """A trainable wrapper for a GPflow Gaussian process model."""

    def __init__(self, model:GPModel, fantasized_data:Dataset):
        """
        :param optimizer: The optimizer with which to train the model. Defaults to
            :class:`~trieste.models.optimizer.Optimizer` with :class:`~gpflow.optimizers.Scipy`.
        """
    ...
    def predict(self, query_points: TensorType) -> tuple[TensorType, TensorType]:
        return self.model.fantasized_predict_f(query_points, self._fantasized_data)

    def predict_joint(self, query_points: TensorType) -> tuple[TensorType, TensorType]:
        return self.model.fantasized_predict_f(query_points, self._fantasized_data, full_cov=True)

    def sample(self, query_points: TensorType, num_samples: int) -> TensorType:
        return self.model.fantasized_predict_f_samples(query_points, self._fantasized_data, num_samples)

    def predict_y(self, query_points: TensorType) -> tuple[TensorType, TensorType]:
        return self.model.fantasized_predict_y(query_points, self._fantasized_data)
        ...
        if summary_writer:
            with summary_writer.as_default(step=get_step_number()):
                ...
                if tf.rank(lengthscales) == 0:
                    ...
                    for i, lengthscale in enumerate(lengthscales):
                        tf.summary.scalar(f"kernel.lengthscale.{i}", lengthscale)
            # TODO: add log of fantasized points