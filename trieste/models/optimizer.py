...
DatasetBuilder = Callable[[Dataset, Optional[int]], Union[Iterable, Tuple]]
"""
Type alias for a function that creates minibatches from a :class:`~optiflow.datasets.Dataset`.
"""
...
def _create_tf_optimizer(
    optimizer: tf.optimizers.Optimizer, optimizer_args: Dict[str, Any],
) -> Optimizer:
...
def _create_scipy_optimizer(
    optimizer: gpflow.optimizers.Scipy, optimizer_args: Dict[str, Any],
) -> Optimizer:
...
@singledispatch
def create_loss_function(model, dataset: Data, compile: Optional[bool] = False) -> Callable:
    pass


@create_loss_function.register
def _create_loss_function_internal(
    model: InternalDataTrainingLossMixin, data: Data, compile: Optional[bool] = False,
) -> Callable:
...
def _create_loss_function_external(
    model: ExternalDataTrainingLossMixin, data: Data, compile: Optional[bool] = False,
) -> Callable:
