...
def sample_gidgp(model: GIDeepGP) -> Sample:
    class GISample(Sample):
        u_list = [
            tf.Variable(tf.ones([1, 0, 0, 0], dtype=tf.float64), shape=[1, None, None, None])
        ...
        def __call__(self, X_new: TensorType) -> tf.Tensor:
            ...
            for i, layer in enumerate(model.f_layers):
                ...
                Kuu, Kuf, Kfu = (
                    tf.expand_dims(Kuu, 1),
                    tf.expand_dims(Kuf, 1),
                    tf.expand_dims(Kfu, 1),
                )
                ...
                mean, cov = layer.predict(
                    self.u_list[i],
                    Kff,
                    Kuf,
                    self.chol_Kuu_list[i],
                    inputs=tf.concat([inducing_data, x], 1),
                    full_cov=True,
                )
                mean = tf.linalg.matrix_transpose(mean)  # [1, P, N_old + N_new]
                f_old = tf.linalg.matrix_transpose(self.f[i])  # [P, N_old]
                f_new = draw_conditional_sample(
                    mean[0], tf.tile(cov, [self.P[i], 1, 1]), f_old
                )  # [P, N_new]
