...
class ProbabilityOfImprovement(SingleModelAcquisitionFunction[ProbabilisticModel]):
    """
    ...
    def __init__(self, model: ProbabilisticModel, dataset: Optional[Dataset] = None) -> None:
        """
        ...
        self.eta = tf.reduce_min(mean, axis=0)[0]
        self.model = model
    ...
    def __call__(self, x: TensorType) -> TensorType:
        ...
        mean, var = self.model.predict(tf.squeeze(x, -2))
        distr = tfp.distributions.Normal(mean, tf.sqrt(var))
        return distr.cdf(tf.cast(self.eta, x.dtype))
