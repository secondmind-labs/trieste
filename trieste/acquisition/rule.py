...
class EfficientGlobalOptimization(
    ...
    def acquire(
        ...
        with tf.name_scope("EGO.optimizer" + "[0]" * greedy):
            points, self.vectorized_child_results = self._optimizer(search_space, self._acquisition_function)
        ...
        if isinstance(self._builder, GreedyAcquisitionFunctionBuilder):
            for i in range(
                ...
                with tf.name_scope(f"EGO.optimizer[{i+1}]"):
                    chosen_point, self.vectorized_child_results = self._optimizer(search_space, self._acquisition_function)
