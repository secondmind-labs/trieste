...
from typing import Callable, Generic, Optional, TypeVar, Union, cast, overload, Type
...
class EfficientGlobalOptimization(
    ...
    def __init__(
        ...
        initial_acquisition_function: Optional[AcquisitionFunction[ProbabilisticModelType]] = None,
    ):
        ...
    ...
    def __init__(
        ...
        builder: (
            Type[AcquisitionFunction[ProbabilisticModelType]]
    ...
    def __init__(
        ...
        if num_query_points > 1:  # need to build batches of points
            # if issubclass(builder, VectorizedAcquisitionFunctionBuilder):
            #     # optimize batch elements independently
            #     optimizer = batchify_vectorize(optimizer, num_query_points)
            if issubclass(builder, AcquisitionFunction):
                # optimize batch elements jointly
                optimizer = batchify_joint(optimizer, num_query_points)
            # elif issubclass(builder, GreedyAcquisitionFunctionBuilder):
            #     # optimize batch elements sequentially using the logic in acquire.
            #     pass

        self._builder: Type[AcquisitionFunction[ProbabilisticModelType]] = builder
        self._optimizer = optimizer
        self._num_query_points = num_query_points
        self._acquisition_function = initial_acquisition_function
    ...
    @property
    def acquisition_function(self) -> Optional[AcquisitionFunction[ProbabilisticModelType]]:
    ...
    def acquire(
        ...
        if self._acquisition_function is None:
            self._acquisition_function = self._builder(
                models,
                datasets=datasets,
            )
        else:
            self._acquisition_function = self._acquisition_function.update(
                models,
                datasets=datasets,
            )
        ...
        greedy = False  # isinstance(self._builder, GreedyAcquisitionFunctionBuilder)
