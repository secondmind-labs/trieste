...
from gpflux.layers import LatentVariableLayer, KG, IWLayer

from trieste.acquisition.sampler import Sampler
from trieste.models import ProbabilisticModel
from trieste.models.gpflux import DeepGaussianProcess, GlobalInducingDeepGaussianProcess, DeepKernelProcess
...
class DeepKernelProcessSampler(Sampler):
    ...
    def sample(self, at: TensorType) -> TensorType:
        ...
        samples = self._model.model_gpflux._inducing_add(samples)
        samples = self._model.model_gpflux.kernelizer(samples)

        for i, layer in enumerate(self._model.model_gpflux.f_layers[:-1]):
        ...
        layer = self._model.model_gpflux.f_layers[-1]
