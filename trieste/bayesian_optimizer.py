...
from .models import TrainableProbabilisticModel, SupportsCovarianceWithTopFidelity
...
class OptimizationResult(Generic[StateType]):
    ...
    def try_get_optimal_point(self) -> tuple[TensorType, TensorType, TensorType]:
        ...
        if tf.reduce_any([isinstance(model, SupportsCovarianceWithTopFidelity) for model in self.try_get_final_models()]):
