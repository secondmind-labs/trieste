...
@random_seed
def test_deep_gaussian_process_sampler_sample_is_continuous(two_layer_model: Callable) -> None:
    x = tf.random.uniform([100, 2], minval=-10.0, maxval=10.0, dtype=tf.float64)
    y = tf.random.normal([100, 1], dtype=tf.float64)
    dataset = Dataset(x, y)
...
def test_deep_gaussian_process_sampler_sample_is_repeatable(two_layer_model: Callable) -> None:
    x = tf.random.uniform([100, 2], minval=-10.0, maxval=10.0, dtype=tf.float64)
    y = tf.random.normal([100, 1], dtype=tf.float64)
    dataset = Dataset(x, y)
