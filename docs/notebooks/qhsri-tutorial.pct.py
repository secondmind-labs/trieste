...
u_deltas = [
    (
        float(max(optimised_non_dominated[:, i]))
        - float(min(optimised_non_dominated[:, i]))
    )
    * 0.2
    for i in range(2)
]
reference_point = [
    float(max(optimised_non_dominated[:, i])) + u_deltas[i] for i in range(2)
]
...
plt.scatter(
    lower_bound[0], lower_bound[1], c="r", label="lower bound", marker="x"
)
plt.scatter(
    reference_point[0],
    reference_point[1],
...
hypervolume_rectangle = plt.Rectangle(
    first_point, width, height, fc="orange", alpha=0.5
)
...
plt.gca().add_patch(hypervolume_rectangle)
hypervolume_rectangle2 = plt.Rectangle(
    second_point, width2, height2, fc="blue", alpha=0.5
)
...
for i in range(front_size):
    for j in range(front_size):
        p[i, j] = (
            reference_point[0]
            - max(optimised_non_dominated[i, 0], optimised_non_dominated[j, 0])
        ) * (
            reference_point[1]
            - max(optimised_non_dominated[i, 1], optimised_non_dominated[j, 1])
        )
...
prob = cp.Problem(
    cp.Minimize((1 / 2) * cp.quad_form(y, P)), [G @ y >= h, A @ y == b]
)
prob.solve()
