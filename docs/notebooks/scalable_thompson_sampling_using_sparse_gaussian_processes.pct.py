...
#
# We'll use a continuous bounded search space, and build a noisy observer. We then collect an initial design of 15 random points.
...
def noisy_hartmann_6(x:TensorType) -> TensorType: # contaminate observations with Gaussian noise
    return hartmann_6(x)+ tf.random.normal([len(x),1], 0, 0.01, tf.float64) 
...
#
# By plotting the regret achieved by the algorithm, we see the convergence over five BO steps after the initial design (denoted by a vertical line and cross). Each batch of evaluations getting (dots) achieves better objective values than its predecessor.
