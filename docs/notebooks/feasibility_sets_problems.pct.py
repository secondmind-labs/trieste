...
# %%
from trieste.objectives import branin, BRANIN_SEARCH_SPACE, BRANIN_MINIMUM
...
def threshold_search(low, high, target_volume, observer, search_space, tol = 0.5):
    ...
    if high >= low:
        ...
        mid = high - (high - low) / 2
        print(mid)
        volume = get_volume(mid, observer, search_space)
        print(volume)

        volume_up = get_volume(mid + mid/2, observer, search_space)
        ...
        if tf.abs(volume - target_volume) < 0.005:
            return mid
        ...
        # be present in left subarray
            ...
            if positive:
                return threshold_search(mid + mid*0.001, high, target_volume, observer, search_space)
            else:
                return threshold_search(low, mid - mid*0.001, target_volume, observer, search_space)
            # return threshold_search(mid + mid*0.001, high, target_volume, observer, search_space)
...
def get_volume(threshold, observer, search_space):
    ...
    volume = tf.reduce_mean(tf.cast(estimate_data.observations > threshold, dtype=tf.float32))
    return volume
...
def find_threshold(target_volume, observer, search_space, minimum):
    ...
    print(estimate_max)
    # estimate_max += 0.5*estimate_max 
    # estimate_min = tf.reduce_min(estimate_data.observations)
    estimate_min = minimum
    print(estimate_min)

    threshold = threshold_search(estimate_min, estimate_max, target_volume, observer, search_space, tol = 0.5)
...
get_volume(2.03442853, mk_observer(logarithmic_goldstein_price), LOGARITHMIC_GOLDSTEIN_PRICE_SEARCH_SPACE)
