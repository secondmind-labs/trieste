...
function = gramacy_lee
F_MINIMIZER = GRAMACY_LEE_MINIMUM

search_space = GRAMACY_LEE_SEARCH_SPACE

x_array = np.linspace(search_space.lower, search_space.upper, num=100)
...
plt.plot(x_array, y_array)
plt.scatter(initial_data.query_points.numpy(), initial_data.observations.numpy(), c='r', marker='x')
plt.show()
plt.close()

from trieste.models.gpflow import GaussianProcessRegression, build_gpr, SparseVariational, build_svgp

gpflow_model = build_gpr(initial_data, search_space, likelihood_variance=1e-7)
gp_model = GaussianProcessRegression(gpflow_model)

gp_trajectory_fn = gp_model.trajectory_sampler().get_trajectory()

B = 5
candidates = x_array[:, None, :]  # [N, 1, D]
tiled_candidates = tf.tile(candidates, [1, B, 1])  # [N, V, D]

gp_target_func_values = gp_trajectory_fn(tiled_candidates)  # [N, V]

plt.plot(x_array, y_array, 'b', linewidth=3)
plt.plot(x_array, gp_target_func_values.numpy())
plt.scatter(initial_data.query_points.numpy(), initial_data.observations.numpy(), c='r', marker='x')
plt.show()
plt.close()

svgp_gpflow_model = build_svgp(initial_data, search_space, likelihood_variance=1e-7, num_inducing_points=20)
svgp_model = SparseVariational(svgp_gpflow_model, optimizer=trieste.models.optimizer.BatchOptimizer(tf.optimizers.Adam(1e-2), max_iter=10_000))
svgp_model.optimize(initial_data)

svgp_trajectory_fn = svgp_model.trajectory_sampler().get_trajectory()
svgp_target_func_values = svgp_trajectory_fn(tiled_candidates)

plt.plot(x_array, y_array, 'b', linewidth=3)
plt.plot(x_array, svgp_target_func_values.numpy())
plt.scatter(initial_data.query_points.numpy(), initial_data.observations.numpy(), c='r', marker='x')
plt.show()
plt.close()
...
#
# dgp_trajectory_fn = dgp_model.trajectory_sampler().get_trajectory()
#
...
# gp_trajectory_fn = gp_model.trajectory_sampler().get_trajectory()
#
# num_initial_samples = 133
# V = 1
# candidates = search_space.sample(num_initial_samples)[:, None, :]  # [num_initial_samples, 1, D]
# tiled_candidates = tf.tile(candidates, [1, V, 1])  # [num_initial_samples, V, D]
#
# gp_target_func_values = gp_trajectory_fn(tiled_candidates)  # [num_samples, V]
#
# dgp_target_func_values = dgp_trajectory_fn(tiled_candidates)  # [num_samples, V] ?
#
# model = dgp_model
#
# # from trieste.models.gpflow import GaussianProcessRegression, build_gpr
...
# from trieste.acquisition.function import GreedyContinuousThompsonSampling, ParallelContinuousThompsonSampling
#
# bo = trieste.bayesian_optimizer.BayesianOptimizer(observer, search_space)
# grid_size = 1000
# acquisition_rule = EfficientGlobalOptimization(GreedyContinuousThompsonSampling(), num_query_points=2, )
